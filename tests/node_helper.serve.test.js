...
describe('socketNotificationReceived', () => {
  it('dispatches INITIALIZE, SCAN, and SERVE', async () => {
...
describe('serve', () => {
  it('sends SERVED payload on successful download and metadata', async () => {
    const helper = createHelperState({
      exifrStub: {
  ...
  it('falls back to /files/download when thumbnail download fails', async () => {
    const helper = createHelperState({
      exifrStub: {
  ...
  it('falls back to original download for unsupported thumbnail size', async () => {
    const helper = createHelperState({
      exifrStub: {
        parse: async () => ({ latitude: 1, longitude: 2 }),
      },
  ...
  it('sends SERVE_FAILED when download response is not successful', async () => {
    const helper = createHelperState({
      exifrStub: {
        parse: async () => null,
      },
  ...
  it('sends SERVE_FAILED when both metadata and exif location are missing', async () => {
    const helper = createHelperState({
      exifrStub: {
        parse: async () => null,
      },
  ...
  it('reuses cached reverse geocode results for identical coordinates', async () => {
    const helper = createHelperState({
      exifrStub: {
