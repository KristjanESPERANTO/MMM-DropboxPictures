...
describe('dropboxRequest', () => {
  ...
  it('throws API error when 401 and no refresh_token', async () => {
    const helper = createHelperState()
    const originalFetch = global.fetch
  ...
  it('uses content API base when requested', async () => {
    const helper = createHelperState()
    const originalFetch = global.fetch
...
describe('dropboxDownload', () => {
  ...
  it('throws detailed error for non-401 response', async () => {
    const helper = createHelperState()
    const originalFetch = global.fetch
